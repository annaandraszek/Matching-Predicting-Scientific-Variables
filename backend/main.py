...
def app_user_input(s):
    ...
    if output == 1:
        ...
        if len(properties) > 0 and len(units) <= 0:
            ...
            return ('p', find_more_informative_result(property_predictions, 'property')), ('u', pd.DataFrame())
        if len(units) > 0 and len(properties) <=0:
            unit_predictions = app_run_classifier(units, 'u', unit_model, ranked=True)
            return ('p', pd.DataFrame()), ('u', find_more_informative_result(unit_predictions, 'unit'))
